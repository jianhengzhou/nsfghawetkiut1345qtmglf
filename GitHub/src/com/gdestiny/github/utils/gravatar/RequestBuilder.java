package com.gdestiny.github.utils.gravatar;
...
public class RequestBuilder {
	...
	RequestBuilder(Gravatar gravatar, String email) {
		final String hash = GravatarUtils.convertEmailToHash(email);
		builder = gravatar.ssl ? new StringBuilder(
				GRAVATAR_SECURE_ENDPOINT.length() + hash.length() + 1)
				.append(GRAVATAR_SECURE_ENDPOINT) : new StringBuilder(
				GRAVATAR_ENDPOINT.length() + hash.length() + 1)
				.append(GRAVATAR_ENDPOINT);
		builder.append(hash);
	...
	/**
	 * Specifies the image dimensions (as the image is always squared) to be
	 * retrieved, allowing to save both bandwidth and memory.
	 * <p/>
	 * 
	 * @param sizeInPixels
	 *            must be between {@code Gravatar.MIN_IMAGE_SIZE_PIXEL} and
	 *            {@code Gravatar.MAX_IMAGE_SIZE_PIXEL}.
	 */
	public RequestBuilder size(int sizeInPixels) {
		if (sizeInPixels >= Gravatar.MIN_IMAGE_SIZE_PIXEL
				&& sizeInPixels <= Gravatar.MAX_IMAGE_SIZE_PIXEL) {
			builder.append("&size=").append(sizeInPixels);
			return this;
		}
		throw new IllegalArgumentException(
				"Requested image size must be between "
						+ Gravatar.MIN_IMAGE_SIZE_PIXEL + " and "
						+ Gravatar.MAX_IMAGE_SIZE_PIXEL);
	}
	...
	/**
	 * If the request image couldn't be found, a 404 HTTP error will be returned
	 ...
	 * <p/>
	 * 
	 * @param gravatarDefaultImage
	 *            must be either {@code Gravatar.DefaultImage.MYSTERY_MAN},
	...
	/**
	 * Sets the defaultImage to the custom location pointed by the
	 * {@link String url}.
	 * <p/>
	 * 
	 * @param url
	 *            the default image url, must a accessible over the Internet and
	 *            will be automatically encoded.
	 */
	public RequestBuilder defaultImage(String url) {
		builder.append("&d=").append(GravatarUtils.encode(url));
		return this;
	}
	 ...
	 * 
	 * @param rating
	 *            must be either {@code Gravatar.Rating.g},
	 *            {@code Gravatar.Rating.pg}, {@code Gravatar.Rating.r} or
